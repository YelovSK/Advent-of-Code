...
rows = [row for row, line in enumerate(lines) if all(char == "." for char in line)]
columns = [column for column in range(len(lines[0])) if all(line[column] == "." for line in lines)]
